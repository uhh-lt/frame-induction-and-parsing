...
class MaskedTokenPredictorBert:
    def __init__(self, model, bpe_tokenizer, max_len=300, mask_in_multiunit=False):
        self._model = model
    ...
    def predict_single_unit(self, bpe_tokens, masked_position, 
        ...
        attention_masks_tensor = torch.tensor(token_ids > 0).long().cuda()
        tokens_tensor = torch.tensor(token_ids).cuda()
        
        segments_ids = np.zeros_like(token_ids, dtype=int)
        segments_tensor = torch.tensor(segments_ids).cuda()
        
        self._model.eval()
        target_sent = self._model(tokens_tensor, segments_tensor, 
                                  attention_masks_tensor)[0].detach().cpu().numpy()
