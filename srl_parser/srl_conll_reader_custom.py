...
class SrlConllReaderCustom(DatasetReader):
    def __init__(
        self,
        token_indexers: Dict[str, TokenIndexer] = None,
        bert_model_name: str = None,
        use_predicate: bool = False,
        **kwargs,
    ) -> None:
        ...
        self._use_predicate = use_predicate
    ...
    def _read(self, file_path: str):
        ...
        for sent in sentences:
            ...
            for word in sent:
                word = word.split("\t")
                tokens.append(Token(word[1]))
                tag = word[-1].replace("S-", "B-")
                tags.append(tag)
                verb_indicator.append(1 if word[-2] != "_" else 0)
                if word[-2] != "_":
                    predicate = word[-2]

            yield self.text_to_instance(tokens, verb_indicator, tags, predicate)
    ...
    def text_to_instance(  # type: ignore
        ...
        if self.bert_tokenizer is not None:
            if self._use_predicate:
                tokens += [Token("[SEP]"), Token(predicate)]
                verb_label += [0, 0]
                tags += ["O", "O"]
