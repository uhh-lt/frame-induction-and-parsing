...
from .globalconfig import VERSION, PARSER_DATA_DIR, PARSER_OUTPUT_DIR
...
# !!!change ***********
optpr.add_option("--tagger", type="str", default=None)
optpr.add_option("--version", type="str", default=VERSION)
optpr.add_option("--data_dir", type="str", default=PARSER_DATA_DIR)
optpr.add_option("--exp_name", type="str", default='')
# !!!change ***********
(options, args) = optpr.parse_args()

# !!!change ***********
from sesame.globalconfig import load_experiment_configs
load_experiment_configs(options.exp_name, data_dir=options.data_dir, version=options.version)
from sesame.globalconfig import (VERSION, TRAIN_EXEMPLAR, TRAIN_FTE, DEV_CONLL, TEST_CONLL,
                          FULLTEXT_DIR, PARSER_DATA_DIR, TEST_FILES, DEV_FILES, LU_DIR, EMBEDDINGS_FILE)
# !!!change ***********
...
#             outf.write(str(token.encode('utf-8')) + "\t")  # FORM = 1
            outf.write(token.encode('utf-8').decode() + "\t")  # FORM = 1
    ...
    for anno in sent.findall('fn:annotationSet', ns):
        for layer in anno.findall('fn:layer', ns):
            ...
            if layertype in relevantposlayers:
                ...
                if sentann.normalize_tokens(logger, tagger=options.tagger) is None:
