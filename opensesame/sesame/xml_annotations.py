...
class SentenceAnnotation(object):
    ...
    def normalize_tokens(self, logger, tagger=None):
        ...
        if tagger == 'pattern':
            try:
                sentence = parse([self.tokens], tokenize=False, tags=True, chunks=False, lemmata=True).split()[0]

                self.nltkpostags = [token[1].split('-')[0] for token in sentence]
                self.nltklemmas = [token[2] for token in sentence]
