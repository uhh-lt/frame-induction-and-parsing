...
from .globalconfig import TEST_CONLL
from .globalconfig import VERSION, PARSER_DATA_DIR, PARSER_OUTPUT_DIR, ARGID_LR

default_testf = TEST_CONLL
...
# !!!change ***********
if options.test_conll == default_testf: options.test_conll = TEST_CONLL
...
model_file_best = "{}best-argid-{}-model".format(model_dir, VERSION)
model_file_name = "{}argid-{}-model".format(model_dir, VERSION)
...
logger = create_logger("argid_log", model_dir, level=logging.INFO)
...
ARGID_LR = configuration["lr"]

OLD_SIZE = len(trainexamples)
...
logger.info(f"TRAIN_SET size \t{OLD_SIZE}")
logger.info(f"TRAIN_SET size(after filtering max span length) \t{len(trainexamples)}")
...
if USE_PTB_CONSTITS:
    ...
    logger.info("weighing PTB down by %f\n" % DELTA)
...
def get_loss(factorexprs, gold_fes, valid_fes, sentlen):
    ...
    if partition.scalar_value() < numerator.scalar_value():
        logger.info("WARNING: partition ~~ numerator! possibly overfitting difference = %f\n"
                         % lossexp.scalar_value())
        return None

    if lossexp.scalar_value() < 0.0:
        logger.info(str(gold_fes) + "\ngolds\n")
        gsum = 0
        for fac in goldfactors:
            gsum += factorexprs[fac].scalar_value()
            logger.info(fac.to_str(FEDICT) + " " + str(factorexprs[fac].scalar_value()) + "\n")
        logger.info("my calculation = " + str(gsum) + " vs " + str(numerator.scalar_value()) + "\n")
        for j in range(sentlen):
            logger.info(":" + str(j) + "\t")
            if not USE_SPAN_CLIP or j <= ALLOWED_SPANLEN:
                logger.info("0 ")
...
def identify_fes(unkdtoks, sentence, tfdict, goldfes=None, testidx=None, trainmode=False):
    dy.renew_cg()
...
def print_eval_result(examples, expredictions, prediction_logger):
    ...
    corp_ures, corp_labldres, corp_tokres = evaluate_corpus_argid(
        examples, expredictions, corefrmfemap, NOTANFEID, prediction_logger)
...
def evaluate_model(dev_iterator):                
    ...
    return (lp, lr, lf), (ures, tokenwise), predictions    



prediction_logger = open("{}/argid-prediction-analysis.log".format(model_dir), "w")
...
#                          'eval_loss':[],
    ...
    if options.mode in ["refresh"]:
        ...
        if log_tensorboard:
            for key in ['training_loss', 'eval_f1']:
            ...
            for key in ['training_loss', 'train_f1']:
                simple_key = key.split('_')[1] if key!="training_loss" else key
                for i in range(len(df)):    
...
#         !!!change ***********
        ...
        for idx, trex in enumerate(train_iterator, 1):
            ...
            if idx % 100 == 0:
                ...
                logger.info(f"step :{steps_trained} - time: {elapsed}")
                ttime = time.time()
            train_iterator.set_description(
                "epoch = %d steps=%d loss = %.4f val_Lf1 = %.4f best_val_Lf1 = %.4f" % (
                    epoch+1, global_step, loss/idx, lf, best_dev_f1))
...
#                 training_progress['eval_loss'].append(dev_loss)
                training_progress['train_f1'].append(train_lf)
                training_progress['eval_f1'].append(lf)
...
# !!!change ***********
    ...
    logger.info("reading ensemble factors...")
    ...
    logger.info("testing " + str(len(devexamples)) + " examples ...\n")

    testpredictions = []
    for tidx, testex in enumerate(devexamples, 1):
        if tidx % 100 == 0:
            logger.info(str(tidx) + "...")
    ...
    logger.info("done!\n")
    print_eval_result(devexamples, testpredictions, prediction_logger)
    logger.info("printing frame-elements to " + fe_file + " ...\n")
